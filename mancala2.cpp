...
void adversarialSearch::fill_up_board()
{
    ...
    //cout<<"board size:"<<board2.size()<<endl;
...
int * adversarialSearch::checkeval(map<int,int> board, int index, int turn)
{
    ...
    int p1_man = (size/2)-1;
    int p2_man = size-1;
    //cout<<"p1_man "<<final_state_root[p1_man]<<endl;
    //cout<<"p2_man "<<final_state_root[p2_man]<<endl;
    if(turn == 1)
    {
        start = 0;
        end = size/2 -1;
    ...
    *(eval+0) = j;
    if(this->p_turn == 1)
    {
        evalu = abs((rand()%21+100)*(temp[p1_man] - temp[p2_man])+(rand()%21+100)*((p1_man-temp[p1_man])-(p2_man-temp[p2_man]))+(rand()%21+100)*(j-1)+(rand()%21+100)*final_state_root[p1_man]);
        //cout<<"evalution 1 : "<<evalu<<endl;
    }
    else
    {
        //evalu = temp[p2_man] - temp[p1_man];
        evalu = abs((rand()%21+100)*(temp[p2_man] - temp[p1_man]) + (rand()%21+100)*((p2_man-temp[p2_man])-(p1_man-temp[p1_man])));
        //cout<<"evalution 2 : "<<evalu<<endl;
    }
    *(eval+1) = evalu;
    return eval;
}
...
int adversarialSearch::max_value(map<int,int> board,int a_depth, string name,int pos,int retainedTurn,int player, ofstream& answer)
{
    ...
    if((checkendcondition) == 1)
    {
        ...
        //cout << name<<","<<a_depth<<","<<estimate_value(val1)<<endl;
        return val1;
    }
    ...
    if(this->c_o_depth == a_depth)
    {
        ...
        if(retainedTurn)
        {
            ...
            //cout << name<<","<<a_depth<<","<<estimate_value(value)<<endl;
        }
...
int adversarialSearch::max_value_alphabeta(map<int,int> board,int a_depth, string name,int pos,int retainedTurn,int alpha, int beta,int player,ofstream& answer)
{
    ...
    if((checkendcondition) == 1)
    {
        ...
        //	cout <<name<<","<<a_depth<<","<<estimate_value(val1)<<","<<estimate_value(alpha)<<","<<estimate_value(beta)<<endl;
        return val1;
    }
    ...
    if(this->c_o_depth == a_depth)
    {
        ...
        if(retainedTurn)
        {
            ...
            //    cout <<name<<","<<a_depth<<","<<estimate_value(value)<<","<<estimate_value(alpha)<<","<<estimate_value(beta)<<endl;
        }
...
int main(int argc, char **argv)
{
    ...
    for(int num=1; num<=10; num++)
    {
        ...
        //cout<<"final state root1 "<<final_state_root.size()<<endl;
        outfile1 << p1_man<<endl;
        if(final_state_root[i]<p1_man)
        {
            win_ratio++;
        }
        outfile1.close();
    ...
    cout<<"player1 wins : "<<(win_ratio/10)*100<<endl;
    cout<<"player2 wins : "<<((10-win_ratio)/10)*100<<endl;
